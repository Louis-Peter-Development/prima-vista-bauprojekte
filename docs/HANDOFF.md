...
- Status: **implemented 2026-07-18** (fully automatic, per Louis' decision). How it works:
  ...
  - Pricing (`server/blitzEstimate.ts`): (a) calculator handoffs — grand total re-derived server-side from sanitized line items (client totals ignored); (b) direct Haus-/Wohnungs-/Gastronomie-/Büro package requests with plausible m² — rebuilt from trusted Standard/default configurations in `server/blitzRates.generated.ts`, using the exact calculator quantity rules at the entered area (regenerate: `node scripts/generate-blitz-rates.mjs`). Both automatic paths attach an itemized PDF. Direct defaults are `1e`, `2zi`, Restaurant, and Klassisches Büro; house/apartment use the 0.9–1.15 calculator band, restaurant/office 0.85–1.20. Corridor €3k–900k; single trades, heating, and odd areas ⇒ manual review.
...
- Status: **implemented 2026-07-18** (full Google Calendar integration, per Louis' decision). How it works:
  - `/kontakt` form gained an optional **Wunschtermin** section: custom localized month-grid picker (`src/components/kontakt/DatePickerField.tsx`) — no past dates, no Sundays, 6-month horizon — plus exact two-hour appointments (30-minute start increments inside 08:00–18:00), type (vor Ort/Video), and a collapsible alternative date with its own exact time. Clearly framed as a preference; "Termin in 48 Stunden" confirmation promise unchanged.
  - Availability: `GET /api/termine?month=` (`netlify/functions/termine.ts` → `server/terminAvailability.ts`) exposes **available start times only**, derived from Google free/busy. Every option that overlaps an existing calendar event is removed; no event titles, attendees, descriptions, or other private details reach the browser. Cached 60 seconds server-side and 30 seconds at the edge/client, rate-limited.
  - Submission: the server re-checks the exact slot against live Google free/busy to catch a stale selection. A successful appointment request creates a **tentative, opaque timed event** first, reserving the precise two-hour period in the linked calendar; confirmation emails then include the exact start/end time. If email delivery fails, the calendar event is rolled back. If calendar availability cannot be checked safely, the appointment submission fails closed instead of risking an overlap.
