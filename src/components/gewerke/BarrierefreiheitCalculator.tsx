...
import {
  BARRIEREFREIHEIT_EXTRAS,
  BARRIEREFREIHEIT_VARIANTS,
  formatEuro,
  type BarrierefreiheitVariantKey,
} from '../../data/barrierefreiheit';

type ExtraKey = (typeof BARRIEREFREIHEIT_EXTRAS)[number]['key'];
...
export default function BarrierefreiheitCalculator({ embedded }: { embedded?: boolean } = {}) {
  ...
  const variant = BARRIEREFREIHEIT_VARIANTS.find((item) => item.key === variantKey) ?? BARRIEREFREIHEIT_VARIANTS[0];

  const lines = useMemo(() => {
    const areaFactor = Math.max(0.75, area / 8);
    const base: CalcLine[] = [
      {
        key: variant.key,
        label: variant.title,
    ...
    BARRIEREFREIHEIT_EXTRAS.forEach((extra) => {
      if (!extras[extra.key]) return;
      base.push({
        key: extra.key,
        label: extra.label,
        detail: 'Optionale Position aus dem Barrierefreies-Bad-Kalkulator.',
        qty: 1,
        unit: 'Paket',
      ...
      <div className="hk-calc__inner">
        ...
        <div className="hk-product reveal">
          ...
          <div className="hk-product__copy">
            ...
            <p>
              Bodengleiche Dusche, unterfahrbarer Waschtisch, barrierefreies WC, Haltegriffe und rutschfeste Beläge ergeben direkt eine Vorab-Schätzung.
            </p>
          </div>
        </div>
        )}
        ...
        <div className="hk-config reveal" data-delay="1">
          <div className="hk-config__field">
            ...
            <div className="hk-meter hk-meter--wide">
              <label htmlFor="barrierefreiheit-area">Menge</label>
              <input
                id="barrierefreiheit-area"
                type="range"
                min={3}
                max={24}
                step={1}
            ...
            <div className="hk-stepper hk-stepper--compact">
              <button type="button" onClick={() => setArea((value) => Math.max(3, value - 1))} aria-label="Badfläche reduzieren">-</button>
              <input
                type="number"
                min={3}
                max={24}
              ...
              />
              <button type="button" onClick={() => setArea((value) => Math.min(24, value + 1))} aria-label="Badfläche erhöhen">+</button>
            </div>
          </div>
          ...
          <div className="hk-config__field">
            <div className="hk-config__head">
              <span>02</span>
              <strong>Barrierefreie Leistung</strong>
              <em>{BARRIEREFREIHEIT_VARIANTS.length} Varianten</em>
            </div>
            <div className="hk-options">
              {BARRIEREFREIHEIT_VARIANTS.map((item) => (
                <button
                  key={item.key}
                  type="button"
          ...
          <div className="hk-config__field">
            ...
            <div className="hk-toggle-list">
              {BARRIEREFREIHEIT_EXTRAS.map((extra) => (
        ...
        <aside className="hk-summary reveal" data-delay="2">
          <div className="hk-summary__sticky">
            ...
            <p>
              Vorab-Schätzung nach dem importierten Barrierefreies-Bad-Kalkulator. Verbindlich wird der Preis nach Aufmaß, Bewegungsflächen, Türbreiten, Untergrund, Sanitäranschlüssen und Montagehöhen.
            </p>
