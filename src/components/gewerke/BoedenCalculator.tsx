...
import { BODEN_EXTRAS, BODEN_VARIANTS, formatEuro, type BodenVariantKey } from '../../data/boeden';

type ExtraKey = (typeof BODEN_EXTRAS)[number]['key'];
...
export default function BoedenCalculator({ embedded }: { embedded?: boolean } = {}) {
  const [variantKey, setVariantKey] = useState<BodenVariantKey>('all');
  const [area, setArea] = useState(45);
  const [extras, setExtras] = useState<ExtrasState>(defaultExtras);

  const variant = BODEN_VARIANTS.find((item) => item.key === variantKey) ?? BODEN_VARIANTS[0];
  ...
  const lines = useMemo(() => {
    ...
    BODEN_EXTRAS.forEach((extra) => {
      if (!extras[extra.key]) return;
      base.push({
        key: extra.key,
        label: extra.label,
        detail: 'Optionale Position aus dem Boden- und Estrichpaket.',
        qty: 1,
        unit: 'Paket',
  ...
  return (
    <section className="hk-calc hk-calc--boeden" aria-label="Bodenverlegung Kostenrechner">
      <div className="hk-calc__inner">
        ...
        <div className="hk-product reveal">
          ...
          <div className="hk-product__copy">
            ...
            <p>
              Parkett, Laminat, Vinyl, Fliesen, Teppich oder Estrich: Fläche, Belag und Zusatzarbeiten werden direkt zu einer Vorab-Schätzung zusammengeführt.
            </p>
          </div>
        </div>
        )}
        ...
        <div className="hk-config reveal" data-delay="1">
          <div className="hk-config__field">
            <div className="hk-config__head">
              <span>01</span>
              <strong>Bodenfläche</strong>
              <em>{area} qm</em>
            </div>
            <div className="hk-meter hk-meter--wide">
              <label htmlFor="boden-area">Menge</label>
              <input
                id="boden-area"
                type="range"
                min={8}
            ...
            <div className="hk-stepper hk-stepper--compact">
              <button type="button" onClick={() => setArea((value) => Math.max(8, value - 1))} aria-label="Bodenfläche reduzieren">-</button>
              <input
                type="number"
                min={8}
                max={160}
                value={area}
                onChange={(event) => setArea(Math.min(160, Math.max(8, Number(event.target.value) || 8)))}
                aria-label="Bodenfläche"
              />
              <button type="button" onClick={() => setArea((value) => Math.min(160, value + 1))} aria-label="Bodenfläche erhöhen">+</button>
            </div>
          </div>
          ...
          <div className="hk-config__field">
            <div className="hk-config__head">
              <span>02</span>
              <strong>Belag & Leistung</strong>
              <em>{BODEN_VARIANTS.length} Varianten</em>
            </div>
            <div className="hk-options">
              {BODEN_VARIANTS.map((item) => (
                <button
                  key={item.key}
                  type="button"
        ...
        <aside className="hk-summary reveal" data-delay="2">
          <div className="hk-summary__sticky">
            ...
            <p>
              Vorab-Schätzung nach Varianten aus dem importierten Bodenverlegung-Kalkulator. Verbindlich wird der Preis nach Aufmaß, Untergrundprüfung, Materialwahl und Anschlussdetails.
            </p>
