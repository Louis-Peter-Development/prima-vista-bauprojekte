...
import { TROCKENBAU_EXTRAS, TROCKENBAU_VARIANTS, formatEuro, type TrockenbauVariantKey } from '../../data/trockenbau';

type ExtraKey = (typeof TROCKENBAU_EXTRAS)[number]['key'];
...
export default function TrockenbauCalculator() {
  ...
  const variant = TROCKENBAU_VARIANTS.find((item) => item.key === variantKey) ?? TROCKENBAU_VARIANTS[0];

  const lines = useMemo(() => {
    const areaFactor = Math.max(0.6, area / 35);
    const base: CalcLine[] = [
      {
        key: variant.key,
        label: variant.title,
    ...
    TROCKENBAU_EXTRAS.forEach((extra) => {
      if (!extras[extra.key]) return;
      base.push({
        key: extra.key,
        label: extra.label,
        detail: 'Optionale Position aus dem Trockenbau- und Innenausbaupaket.',
        qty: 1,
        unit: 'Paket',
      ...
      <div className="hk-calc__inner">
        <div className="hk-product reveal">
          ...
          <div className="hk-product__copy">
            ...
            <p>
              Decken, Wände, Verkleidungen, Estrich oder Dachschrägen: Fläche, Ausführung und Zusatzpositionen ergeben direkt eine Vorab-Schätzung.
            </p>
          </div>
        </div>
        ...
        <div className="hk-config reveal" data-delay="1">
          <div className="hk-config__field">
            ...
            <div className="hk-meter hk-meter--wide">
              <label htmlFor="trockenbau-area">Menge</label>
              <input
                id="trockenbau-area"
                type="range"
                min={8}
            ...
            <div className="hk-stepper hk-stepper--compact">
              ...
              <input
                ...
                max={140}
                value={area}
                onChange={(event) => setArea(Math.min(140, Math.max(8, Number(event.target.value) || 8)))}
                aria-label="Trockenbaufläche"
              />
              <button type="button" onClick={() => setArea((value) => Math.min(140, value + 1))} aria-label="Fläche erhöhen">+</button>
            </div>
          </div>
          ...
          <div className="hk-config__field">
            <div className="hk-config__head">
              <span>02</span>
              <strong>Trockenbauleistung</strong>
              <em>{TROCKENBAU_VARIANTS.length} Varianten</em>
            </div>
            <div className="hk-options">
              {TROCKENBAU_VARIANTS.map((item) => (
                <button
                  key={item.key}
                  type="button"
        ...
        <aside className="hk-summary reveal" data-delay="2">
          <div className="hk-summary__sticky">
            ...
            <p>
              Vorab-Schätzung nach Varianten aus dem importierten Trockenbau-Kalkulator. Verbindlich wird der Preis nach Aufmaß, Untergrund, Anschlüssen, Dämmung und Oberflächenqualität.
            </p>
