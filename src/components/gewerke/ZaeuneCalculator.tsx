...
import { ZAUN_EXTRAS, ZAUN_VARIANTS, formatEuro, type ZaunVariantKey } from '../../data/zaeune';

type ExtraKey = (typeof ZAUN_EXTRAS)[number]['key'];
...
export default function ZaeuneCalculator({ embedded }: { embedded?: boolean } = {}) {
  const [variantKey, setVariantKey] = useState<ZaunVariantKey>('all');
  const [length, setLength] = useState(40);
  const [extras, setExtras] = useState<ExtrasState>(defaultExtras);

  const variant = ZAUN_VARIANTS.find((item) => item.key === variantKey) ?? ZAUN_VARIANTS[0];

  const lines = useMemo(() => {
    const lengthFactor = Math.max(0.4, length / 40);
    const base: CalcLine[] = [
      {
        key: variant.key,
        label: variant.title,
    ...
    ZAUN_EXTRAS.forEach((extra) => {
      if (!extras[extra.key]) return;
      base.push({
        key: extra.key,
        label: extra.label,
        detail: 'Optionale Position aus dem Zaun-Kalkulator.',
        qty: 1,
        unit: 'Paket',
  ...
  return (
    <section className="hk-calc hk-calc--zaeune" aria-label="Zäune Kostenrechner">
      <div className="hk-calc__inner">
        ...
        <div className="hk-product reveal">
          ...
          <div className="hk-product__copy">
            ...
            <p>
              Doppelstab, Holz, Sichtschutz oder Maschendraht: Zaunlänge, Bauart und Zusatzpositionen ergeben direkt eine Vorab-Schätzung.
            </p>
          </div>
        </div>
        )}
        ...
        <div className="hk-config reveal" data-delay="1">
          <div className="hk-config__field">
            <div className="hk-config__head">
              <span>01</span>
              <strong>Zaunlänge</strong>
              <em>{length} lfm</em>
            </div>
            <div className="hk-meter hk-meter--wide">
              <label htmlFor="zaun-length">Länge</label>
              <input
                id="zaun-length"
                type="range"
                min={8}
            ...
            <div className="hk-stepper hk-stepper--compact">
              <button type="button" onClick={() => setLength((value) => Math.max(8, value - 1))} aria-label="Zaunlänge reduzieren">-</button>
              <input
                type="number"
                min={8}
                max={200}
                value={length}
                onChange={(event) => setLength(Math.min(200, Math.max(8, Number(event.target.value) || 8)))}
                aria-label="Zaunlänge"
              />
              <button type="button" onClick={() => setLength((value) => Math.min(200, value + 1))} aria-label="Zaunlänge erhöhen">+</button>
            </div>
          </div>
          ...
          <div className="hk-config__field">
            <div className="hk-config__head">
              <span>02</span>
              <strong>Zaunart</strong>
              <em>{ZAUN_VARIANTS.length} Varianten</em>
            </div>
            <div className="hk-options">
              {ZAUN_VARIANTS.map((item) => (
                <button
                  key={item.key}
                  type="button"
        ...
        <aside className="hk-summary reveal" data-delay="2">
          <div className="hk-summary__sticky">
            ...
            <p>
              Vorab-Schätzung nach Varianten aus dem importierten Zaun-Kalkulator. Verbindlich wird der Preis nach Aufmaß, Geländeprofil, Bodenklasse, Material und Toranlage.
            </p>
