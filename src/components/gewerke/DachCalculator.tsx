...
import { DACH_EXTRAS, DACH_VARIANTS, formatEuro, type DachVariantKey } from '../../data/dach';

type ExtraKey = (typeof DACH_EXTRAS)[number]['key'];
...
export default function DachCalculator({ embedded }: { embedded?: boolean } = {}) {
  ...
  const variant = DACH_VARIANTS.find((item) => item.key === variantKey) ?? DACH_VARIANTS[0];

  const lines = useMemo(() => {
    const areaFactor = Math.max(0.5, area / 110);
    const base: CalcLine[] = [
      {
        key: variant.key,
        label: variant.title,
    ...
    DACH_EXTRAS.forEach((extra) => {
      if (!extras[extra.key]) return;
      base.push({
        key: extra.key,
        label: extra.label,
        detail: 'Optionale Position aus dem Dachsanierung-Kalkulator.',
        qty: 1,
        unit: 'Paket',
      ...
      <div className="hk-calc__inner">
        ...
        <div className="hk-product reveal">
          ...
          <div className="hk-product__copy">
            ...
            <p>
              Neubedachung, Dachstuhl, Dämmung, Gauben oder Dachfenster: Fläche, Leistung und Zusatzpositionen ergeben direkt eine Vorab-Schätzung.
            </p>
          </div>
        </div>
        )}
        ...
        <div className="hk-config reveal" data-delay="1">
          <div className="hk-config__field">
            ...
            <div className="hk-meter hk-meter--wide">
              <label htmlFor="dach-area">Menge</label>
              <input
                id="dach-area"
                type="range"
                min={25}
            ...
            <div className="hk-stepper hk-stepper--compact">
              <button type="button" onClick={() => setArea((value) => Math.max(25, value - 5))} aria-label="Dachfläche reduzieren">-</button>
              <input
                type="number"
                min={25}
                max={320}
                value={area}
                onChange={(event) => setArea(Math.min(320, Math.max(25, Number(event.target.value) || 25)))}
                aria-label="Dachfläche"
              />
              <button type="button" onClick={() => setArea((value) => Math.min(320, value + 5))} aria-label="Dachfläche erhöhen">+</button>
            </div>
          </div>
          ...
          <div className="hk-config__field">
            <div className="hk-config__head">
              <span>02</span>
              <strong>Dachleistung</strong>
              <em>{DACH_VARIANTS.length} Varianten</em>
            </div>
            <div className="hk-options">
              {DACH_VARIANTS.map((item) => (
                <button
                  key={item.key}
                  type="button"
        ...
        <aside className="hk-summary reveal" data-delay="2">
          <div className="hk-summary__sticky">
            ...
            <p>
              Vorab-Schätzung nach Varianten aus dem importierten Dachsanierung-Kalkulator. Verbindlich wird der Preis nach Aufmaß, Dachneigung, Statik, Eindeckung, Dämmung und Anschlussdetails.
            </p>
