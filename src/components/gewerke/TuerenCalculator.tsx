...
import { TUEREN_EXTRAS, TUEREN_VARIANTS, formatEuro, type TuerenVariantKey } from '../../data/tueren';

type ExtraKey = (typeof TUEREN_EXTRAS)[number]['key'];
...
export default function TuerenCalculator({ embedded }: { embedded?: boolean } = {}) {
  ...
  const variant = TUEREN_VARIANTS.find((item) => item.key === variantKey) ?? TUEREN_VARIANTS[0];

  const lines = useMemo(() => {
    const countFactor = Math.max(0.3, count / 6);
    const base: CalcLine[] = [
      {
        key: variant.key,
        label: variant.title,
    ...
    TUEREN_EXTRAS.forEach((extra) => {
      if (!extras[extra.key]) return;
      base.push({
        key: extra.key,
        label: extra.label,
        detail: 'Optionale Position aus dem Türen- & Zargen-Kalkulator.',
        qty: 1,
        unit: 'Paket',
      ...
      <div className="hk-calc__inner">
        ...
        <div className="hk-product reveal">
          ...
          <div className="hk-product__copy">
            ...
            <p>
              Innentüren, Haustür, Schiebetüren oder Nebeneingang: Stückzahl, Türart und Zusatzpositionen ergeben direkt eine Vorab-Schätzung.
            </p>
          </div>
        </div>
        )}
        ...
        <div className="hk-config reveal" data-delay="1">
          <div className="hk-config__field">
            ...
            <div className="hk-meter hk-meter--wide">
              <label htmlFor="tueren-count">Stückzahl</label>
              <input
                id="tueren-count"
                type="range"
                min={1}
            ...
            <div className="hk-stepper hk-stepper--compact">
              <button type="button" onClick={() => setCount((value) => Math.max(1, value - 1))} aria-label="Stückzahl reduzieren">-</button>
              <input
                type="number"
                min={1}
                max={24}
                value={count}
                onChange={(event) => setCount(Math.min(24, Math.max(1, Number(event.target.value) || 1)))}
                aria-label="Stückzahl"
              />
              <button type="button" onClick={() => setCount((value) => Math.min(24, value + 1))} aria-label="Stückzahl erhöhen">+</button>
            </div>
          </div>
          ...
          <div className="hk-config__field">
            <div className="hk-config__head">
              <span>02</span>
              <strong>Türart</strong>
              <em>{TUEREN_VARIANTS.length} Varianten</em>
            </div>
            <div className="hk-options">
              {TUEREN_VARIANTS.map((item) => (
                <button
                  key={item.key}
                  type="button"
        ...
        <aside className="hk-summary reveal" data-delay="2">
          <div className="hk-summary__sticky">
            ...
            <p>
              Vorab-Schätzung nach Varianten aus dem importierten Türen- & Zargen-Kalkulator. Verbindlich wird der Preis nach Aufmaß, Wandstärke, Anschlag, Beschlagswahl und Sicherheitsklasse.
            </p>
