    ...
    <div className="kalk-board">
      <div className="kalk-board__field reveal">
        <div className="kalk-board__field-head">
          ...
          <span className="kalk-board__label">{t('gw.elektro.field')}</span>
          <span className="kalk-board__hint">{t('gw.elektro.hint')}</span>
        </div>
        <div className="haus-types" style={{ flexWrap: 'wrap', gap: '12px' }}>
          {ELEKTRO_TYPES.map((type) => (
            ...
            >
              <span className="haus-types__label">{t(`gw.elektro.types.${type.value}.label`, { defaultValue: type.label })}</span>
              <span className="haus-types__detail" style={{ fontSize: '13px', lineHeight: '1.4' }}>{t(`gw.elektro.types.${type.value}.detail`, { defaultValue: type.detail })}</span>
            </button>
          ))}
        </div>
      </div>
