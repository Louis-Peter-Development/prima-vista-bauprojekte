...
import { FENSTER_EXTRAS, FENSTER_VARIANTS, formatEuro, type FensterVariantKey } from '../../data/fenster';

type ExtraKey = (typeof FENSTER_EXTRAS)[number]['key'];
...
export default function FensterCalculator({ embedded }: { embedded?: boolean } = {}) {
  const [variantKey, setVariantKey] = useState<FensterVariantKey>('all');
  const [count, setCount] = useState(8);
  const [extras, setExtras] = useState<ExtrasState>(defaultExtras);

  const variant = FENSTER_VARIANTS.find((item) => item.key === variantKey) ?? FENSTER_VARIANTS[0];

  const lines = useMemo(() => {
    const countFactor = Math.max(0.5, count / 8);
    const base: CalcLine[] = [
      {
        key: variant.key,
        label: variant.title,
        ...
        qty: count,
        unit: 'Stück',
        totalNet: Math.round(variant.netBase * countFactor),
      },
    ];

    FENSTER_EXTRAS.forEach((extra) => {
      if (!extras[extra.key]) return;
      base.push({
        key: extra.key,
        label: extra.label,
        detail: 'Optionale Position aus dem Fenster- und Balkontüren-Kalkulator.',
        qty: 1,
        unit: 'Paket',
  ...
  return (
    <section className="hk-calc hk-calc--fenster" aria-label="Fenster Kostenrechner">
      <div className="hk-calc__inner">
        ...
        <div className="hk-product reveal">
          ...
          <div className="hk-product__copy">
            ...
            <p>
              Fenster, Balkontüren, Dachfenster, Rollladen, Markisen oder Aufbereitung: Anzahl, Leistung und Zusatzpositionen ergeben direkt eine Vorab-Schätzung.
            </p>
          </div>
        </div>
        )}
        ...
        <div className="hk-config reveal" data-delay="1">
          <div className="hk-config__field">
            ...
            <div className="hk-meter hk-meter--wide">
              <label htmlFor="fenster-count">Menge</label>
              <input
                id="fenster-count"
                type="range"
                min={1}
              ...
              />
              <strong>{count} Stück</strong>
            </div>
            <div className="hk-stepper hk-stepper--compact">
              <button type="button" onClick={() => setCount((value) => Math.max(1, value - 1))} aria-label="Elemente reduzieren">-</button>
              <input
                type="number"
                min={1}
                max={36}
                value={count}
                onChange={(event) => setCount(Math.min(36, Math.max(1, Number(event.target.value) || 1)))}
                aria-label="Fensterelemente"
              />
              <button type="button" onClick={() => setCount((value) => Math.min(36, value + 1))} aria-label="Elemente erhöhen">+</button>
            </div>
          </div>
          ...
          <div className="hk-config__field">
            <div className="hk-config__head">
              <span>02</span>
              <strong>Fensterleistung</strong>
              <em>{FENSTER_VARIANTS.length} Varianten</em>
            </div>
            <div className="hk-options">
              {FENSTER_VARIANTS.map((item) => (
                <button
                  key={item.key}
                  type="button"
        ...
        <aside className="hk-summary reveal" data-delay="2">
          <div className="hk-summary__sticky">
            ...
            <p>
              Vorab-Schätzung nach Varianten aus dem importierten Fenster- und Balkontüren-Kalkulator. Verbindlich wird der Preis nach Aufmaß, Material, Glasaufbau, Anschlussdetails und Sonnenschutz.
            </p>
